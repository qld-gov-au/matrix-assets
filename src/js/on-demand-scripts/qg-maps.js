...
function initMap() {
  $.getScript("./?a=13345", function () {
    //calling markerclusterer.js from Matrix plugins
    ...
    if (searchLat && searchLat[1].length > 0) {
      ...
      center =
        (mapEle.getAttribute("data-center") &&
          mapEle.getAttribute("data-center").split(",")) ||
        "-23,143".split(",");
      zoom = 5;
    }
    ...
    var addMarkerLink = function (item) {
      var template = "",
        wrapper = "";
      var markerUrl =
        window.mapsMarker.replace(/item\[[\s\S]+?\]/g, function (r) {
          return eval(r);
        }) ||
        '<a href="' +
          (
            window.location.origin +
            window.location.pathname.replace(/(_nocache|_recache|_admin)/, "")
          ).replace(/\/$/, "") +
          "/view/?title=" +
